...
const favListElement = document.querySelector(".fav-list") as HTMLDivElement;
const LOADING_SCREEN: HTMLElement = document.querySelector(".fullScreen") as HTMLDivElement;

const locationPromise = USER_LOCATION.getCurrentLocation();
...
// CHECKING FAV ON LOAD //


const checkFavouriteOnLoad = async () => {
    try {
        const favList: SingleFav[] | undefined = await USER_API_SERVICE.GetSingleUserFavourite(`id=${USER.id}`);
        if (favList && favList.length > 0) {
            const locationExists = favList[0].favourites.some(
                (fav: any) => {
                    return (fav as Fav).city === mainCity;
                }
            );
            if (locationExists) {
...
const getFAV = async () => {
    const favList: SingleFav[] | undefined = await USER_API_SERVICE.GetSingleUserFavourite(`id=${USER.id}`);
    if (favList) {
        favList[0].favourites.forEach(async (elem) => {
            const tempDiv = document.createElement('div');
            const city: City[] = await USER_LOCATION.getCoords((elem as Fav).city)
            console.log(city[0].lat);
            //             const element = `
            // <div class="flex items-center px-4 py-2 text-gray-700 hover:bg-blue-50">
            // <i class="fas fa-map-marker-alt mr-2 text-blue-500"></i>
            // <span class="cursor-pointer" onclick="getWeatherAndRender({latitude:${city[0].lat}, longitude:${city[0].lon}})">${(elem as Fav).city}</span>
            // </div>`
            //             tempDiv.innerHTML += element;
            //             favListElement.appendChild(tempDiv.firstElementChild as Node);
        })
    }
}
getFAV()
// COORDINATES AND RENDERING
async function getWeatherAndRender(location: { latitude: number; longitude: number }) {
    ...
    try {
        ...
        const forecastResponse: WeatherForecast | undefined = await WEATHER_SERVICE.GetFiveDayForecast(latitude, longitude);
        const city = await USER_LOCATION.getCityFromCoords(latitude, longitude);
        const time = await GET_TIME.GetTime(latitude, longitude)
        ...
        // GET USER TIME //
        USER_TIME.innerText = `${time.hour}:${time.minute}`;

        // USER DAY AND DATE
        const userDateAndDate = (time: DateTimeInfo) => {
            const getDay = time.day_of_week;
            const getDate = time.day;
            const getMonth = Number.parseFloat(time.month);
            const monthNames = [
                "January", "February", "March", "April", "May", "June",
                "July", "August", "September", "October", "November", "December"
            ];
            const monthName = monthNames[getMonth];
            const day = `${getDay}, ${getDate} ${monthName}`;
            return day;
        }
        const dayAndDate = userDateAndDate(time);
        ...
        try {
            ...
            if (currentWeather) {
                ...
                MAIN_TEMP_FEELS_LIKE.innerText = `Feels like: ${kelvinToCelsius(currentWeather.main.feels_like)}°C`;
...
USER_LOGOUT_BTN.addEventListener("click", () => {
    ...
    window.FlashMessage.success('Logged Out Successfully', { type: 'success', timeout: 1000 });
    setTimeout(() => {
        window.location.href = "login.html";
    }, 1500);
})
...
// handleFavSubmit function //
async function handleFavSubmit(event: Event) {
    event.preventDefault()
    try {
        const user = await USER_API_SERVICE.GetSingleUserFavourite(`id=${USER.id}`);
        if (favouriteBtn.classList.contains("text-white")) {
            let cityAlreadyExists = false;
            if (user && user.length > 0) {
                cityAlreadyExists = user[0].favourites.some(fav => (fav as Fav).city === mainCity);
            }
            if (cityAlreadyExists) {
                window.FlashMessage.warning('This location is already in your favorites', { type: 'warning', timeout: 2000 });
                return;
            }
            favouriteBtn.classList.remove("text-white");
            favouriteBtn.classList.add("text-yellow-500");
            if (!user || user.length === 0) {
                ...
                window.FlashMessage.success('Location added to favorites', { type: 'success', timeout: 2000 });
                await USER_API_SERVICE.PostFavourite(favourite);
            } else {
                const favourite = {
                    "city": mainCity,
                    "coordinates": [mainLocation.latitude, mainLocation.longitude]
                };
                await USER_API_SERVICE.PatchFavourite(favourite, USER.id);
                window.FlashMessage.success('Location added to favorites', { type: 'success', timeout: 2000 });
            }
        } else if (favouriteBtn.classList.contains("text-yellow-500")) {
            favouriteBtn.classList.add("text-white");
            favouriteBtn.classList.remove("text-yellow-500");
            if (user && user.length > 0) {
                const updatedFavourites: SingleFav[] = user[0].favourites.filter((fav) => (fav as Fav).city !== mainCity) as SingleFav[];
                await USER_API_SERVICE.RemovingFavourite(updatedFavourites, USER.id);
                window.FlashMessage.success('Location removed from favorites', { type: 'success', timeout: 2000 });
            }
        }
    } catch (error) {
        console.error("Error handling favorite:", error);
        window.FlashMessage.error('Error updating favorites', { type: 'error', timeout: 2000 });
    }
}
