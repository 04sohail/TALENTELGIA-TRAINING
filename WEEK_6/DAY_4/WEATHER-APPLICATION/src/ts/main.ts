...
const checkFavouriteOnLoad = async () => {
    console.log("INSIDE checkFavouriteOnLoad ");
...
async function handleFavSubmit(event: Event) {
    event.preventDefault()
    console.log("INSIDE handleFavSubmit");
    try {
        const user = await USER_API_SERVICE.GetSingleFavourite(`id=${USER.id}`);
        if (favouriteBtn.classList.contains("text-white")) {
            let cityAlreadyExists = false;
            if (user && user.length > 0) {
                cityAlreadyExists = user[0].favourites.some(fav => fav.city === mainCity);
            }
            ...
            favouriteBtn.classList.remove("text-yellow-500");
            if (user && user.length > 0) {
                const updatedFavourites = user[0].favourites.filter(fav => fav.city !== mainCity);
