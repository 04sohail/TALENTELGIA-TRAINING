...
import { validateAddEmptyField, validateUpdateEmptyField, validateAllAddFieldsInvalid, validateAllUpdateFieldsInvalid } from "./validations";
...
// ADDING USER //
const adminAddSubmitBtn = document.getElementById("addForm") as HTMLFormElement;
adminAddSubmitBtn.addEventListener("submit", (event: Event) => handleAddUser(event));
const handleAddUser = (event: Event): void => {
  event.preventDefault();
...
const showProfile = () => {
  ...
  const user: User | null = sessionStorage.getItem("loggedInSuperAdmin") ? JSON.parse(sessionStorage.getItem("loggedInSuperAdmin") as string) : null;
  console.log(user);
  if (!user) {
