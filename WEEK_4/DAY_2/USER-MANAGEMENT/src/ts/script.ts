...
let savingID: string | undefined = undefined;
...
const patchUser = async (user: Partial<User>) => {
  try {
    LOADING_SCREEN.style.display = "block";
    const API_RESPONSE = await fetch(`${API_URL_USERS}/${user.UID}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
...
const renderUser = async (): Promise<void> => {
  ...
  if (users) {
    ...
    users.forEach((user) => {
      const userRow = `<tr>
        ...
        user.address.pincode
                            ...
                            <td>
      <a href="#editEmployeeModal" class="edit" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Edit" id="edit-${user.UID
        }">&#xE254;</i></a>
      <a href="#deleteEmployeeModal" class="delete deleteUserEvent" data-toggle="modal"><i class="material-icons" id="delete-${user.UID
    ...
    users.forEach((user) => {
      const editBtn = document.getElementById(`edit-${user.UID}`) as HTMLElement;
      const dltBtn = document.getElementById(`delete-${user.UID}`) as HTMLElement;
...
// ADDING USER //
const adminAddSubmitBtn = document.querySelector(".add-admin-submit-btn") as HTMLFormElement;
...
// PROFILE MODAL JS //
const profileModal = document.querySelector("#profileModal") as HTMLElement;
...
const superAdminLogOut = document.querySelector(".super-admin-log-out") as HTMLElement;
