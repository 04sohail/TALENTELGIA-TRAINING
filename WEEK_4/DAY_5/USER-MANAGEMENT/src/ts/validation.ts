class Validation {
    // ON BLUR VALIDATION //
    async OnBlurValidate(event: Event, warningElement: HTMLDivElement): Promise<void> {
        const inputTarget = event.target as HTMLInputElement
        const value: string | null = inputTarget.value?.trim();
        console.log("INSIDE ONBLURVALIDAE");
        if (value.length == 0) {
    ...
    async EditValidateExists(editFields: HTMLInputElement[], userId: null | string | undefined): Promise<boolean> {
        const findEmail = await APIInstance.GetSingleUser(`email=${encodeURIComponent(editFields[2].value)}`);
